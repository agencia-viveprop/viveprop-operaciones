...
## D-098 · Un solo reporte por período, con el grano derivado de la ventana

El usuario pidió rehacer el reporte semanal: dos pestañas, selector de mes arriba, ventana móvil basada en el mes elegido y variable según sus semanas reales, un filtro de 1 a 12 meses hacia atrás para acumular y comparar, y cinco métricas por dominio.

**Antes de construirlo apareció que el reporte mensual ya hacía el 80% de eso**: selector de mes, ventana móvil, dos dominios en un selector, solicitudes, conteos por estado, montos de negocios por tipo y el reparto de la comisión. Lo genuinamente nuevo eran cuatro cosas: el desglose por **etapa**, la **duración en la etapa actual**, la **plata de canjes** y la ventana de **1 a 12** meses. Se le ofrecieron tres caminos y eligió el que evita mantener dos pantallas casi iguales: **ampliar el mensual y retirar el semanal.**

Las otras tres decisiones que tomó: las cuatro casillas de movimiento del semanal --se cerró, avanzó, se cayó, estancado-- y las listas de «Qué pasó» **se van**; en negocios la comisión es la de **ViveProp** y no la de Dataprop, que fue un desliz del enunciado; y el filtro de meses muestra **el acumulado del período con el desglose debajo**.

### El grano se deriva de la ventana, no es un segundo control

Preguntó cómo se combinarían el filtro de semanas y el de meses hacia atrás. La respuesta es que no se combinan:

| Ventana | Desglose |
|---|---|
| 1 mes | las **semanas de ese mes**, cuatro o cinco |
| 2 a 12 meses | **mes a mes** |

Así ninguna combinación queda sin lectura. Semanas por doce meses son 52 barras, y «la semana 5» de un mes que tiene cuatro no existe. Con un solo mes, además, una barra sola no tiene forma: partirla en semanas es lo que hace que el desglose diga algo.

**Las semanas se cuentan desde el día 1** --1 al 7, 8 al 14, y así, con la última cortada donde termina el mes-- y no de lunes a domingo. Con semanas calendario la primera y la última se meten en el mes vecino --agosto de 2026 toca seis semanas ISO-- y eso contradice que la base sea el mes. El costo es que no coinciden con las del reporte semanal viejo.

### La comparación

Un interruptor que superpone otra serie, sin agregar controles: **el tramo inmediatamente anterior del mismo largo** (por defecto) o **el mismo tramo del año pasado**. Las dos responden preguntas distintas --«mejor que el trimestre pasado» y «mejor que el año pasado por esta época»-- y los datos aguantan las dos: hay 15 meses de canjes, de junio de 2025 a agosto de 2026, con entre 6 y 28 solicitudes por mes.

En negocios el interanual va a estar casi vacío y se dijo de antemano: la serie arranca en agosto de 2025 y son 19 liquidaciones en total.

### Canjes ya tiene eje de plata

`METRICAS_CANJES` decía que canjes no tenía plata «y no es un olvido», porque `valor_prop` traía la moneda equivocada en ~138 de las 297 filas migradas: pesos etiquetados UF y al revés (`D-054`). **Esa razón dejó de ser cierta.** Medido sobre los 229 canjes de producción con la regla de magnitud, 228 tienen un valor plausible para su moneda y solo uno no. Así que el eje se calcula con la regla del contrato: 6/5/4% del corretaje en venta según el tramo en UF, 8% en arriendo.

Venta y arriendo van separados y no se suman nunca, por lo mismo que en negocios: un precio de venta y un mes de renta no son la misma unidad. La comisión sí se suma, porque las dos son pesos de comisión.

**`canjes_sin_valorizar` va a la vista.** De los 222 cancelados de producción, 119 se pueden valorizar y 103 no: la serie de UF arranca el 2026-01-01 y esos canjes son de 2025. Un monto bajo por falta de UF no es poca plata, y sin el contador se lee igual.

### Las duraciones son una foto, no una serie

«Cuántos días llevan los canjes abiertos en la etapa en que están» no tiene versión mensual: los abiertos son los de hoy. Por eso ese bloque no entra en la serie y **el `n` va a la vista**: con siete canjes en cuatro etapas, un promedio puede ser de un solo caso y sin el `n` se lee como una tendencia.

El reloj arranca en el movimiento más reciente que dejó al canje **en la etapa que tiene hoy**, no en el último cambio cualquiera: si pasó a «En oferta» y después volvió, importa la última entrada. Sin movimiento que lo diga se mide desde la solicitud y se cuenta en `sin_historia`.

**En negocios el panel va a estar vacío**, y es un dato del estado del proyecto: el pipeline de negocios no tiene ni un movimiento registrado, así que no existe historia de etapas. Se deja construido y explicado en vez de escondido.

### Las etapas se cuentan con la foto de hoy

«De los canjes solicitados en junio, cuántos están hoy en cada etapa». La otra pregunta --en qué etapa estaban al 30 de junio-- pide reconstruir el pasado movimiento por movimiento, y ese dato existe solo desde que el pipeline se usa. Hoy sería una serie inventada.

### El reporte tardaba 56 segundos

La plata de canjes valoriza cada canje con la UF de **su** fecha --la del cierre para un cerrado, la de la solicitud para un cancelado (`D-046`)-- así que hay casi una fecha distinta por canje, y el reporte valoriza los mismos canjes hasta cinco veces: el mes, la ventana, la anterior, el año corrido y su equivalente del año pasado.

Pidiendo la UF una por una eran cientos de consultas contra Neon: **56 segundos**. Con un cache por fecha bajó a 21, que seguía siendo malo porque las fechas son casi todas distintas. La solución fue traer **la serie entera en una consulta** --252 filas-- y resolver en memoria: 6,8 segundos desde mi máquina, contra un baseline de 4 del reporte anterior. La política de qué UF usar no cambió; lo único que se dejó de hacer es preguntar.

`uf_del_canje` documenta el contrato nuevo: cuando se le pasa un cache, **es la serie completa** y una fecha ausente significa «no hay UF para ese día», que es lo mismo que responde `valor_uf` fallando.

### Un defecto viejo que salió al pasar

`test_sin_ningun_dato_la_historica_no_se_cae` llevaba días fallando y pasaba solo mientras el mes mirado fuera el mes en curso: con la base vacía, el primer mes con datos es el mes actual, y mirando agosto desde septiembre la ventana histórica daba cero meses, la serie salía vacía y el promedio reventaba. Ahora la histórica nunca baja de un mes.
