...
**Visitas: importar, ver y borrar de a una.** Pantalla propia para las solicitudes de visita a propiedades que exporta la consola de administración --hoy sin ningún otro lugar en la app--. El archivo no trae un identificador único, así que **cada carga inserta todas las filas como registros nuevos**: subir el mismo archivo dos veces duplica a propósito, y los duplicados se sacan a mano con el borrado de a uno, que es la función que se pidió. El borrado es de rol `operaciones`, no `admin`, porque no hay historial de gestión que perder. El modal de carga trae la misma estructura desplegable y la misma plantilla descargable que Canjes --tampoco se llena a mano, sirve para comparar encabezados cuando la carga falla--, con **solo `Propiedad` marcada como obligatoria** (`D-105`).
