...
**La comisión de Dataprop, en tres tarjetas.** En el Centro de Control van siempre las tres, incluso en cero: **Comisión total** --todos los canjes activos, en cualquier etapa, y cuadra con la casilla de «Activos» de la misma pantalla--, **Potencial desde oferta** --los activos que ya llegaron a la etapa de oferta o más adelante-- y **Realmente cobrada** --lo que se registró al cerrar--. Las dos primeras **no se suman**: la segunda son los mismos canjes de la primera filtrados por etapa, y la diferencia es lo que sigue en revisión o negociando el acuerdo. La **etapa** Cierre no es el **estado** Cerrado --hay 31 canjes cerrando y cero cerrados-- así que el corte los cuenta como activos avanzados. Lo que **no se llegó a cobrar** en los cancelados va en un renglón de referencia y no en tarjeta: estuvo al mismo nivel y con 5 activos contra 224 cancelados era la cifra más grande de la fila, $39,7 millones contra $1,7, que no es comisión de nadie (`D-102`, `D-104`).

**Visitas: importar, ver y borrar.** Pantalla propia para las solicitudes de visita a propiedades que exporta la consola de administración --hoy sin ningún otro lugar en la app--. El archivo no trae un identificador único, así que la carga arma uno propio con **Propiedad + Cliente + RUT + Fecha/hora solicitada**: reimportar el mismo archivo **actualiza** la visita en vez de duplicarla, y de paso refresca `Etapa` y `Corredor`, que son los dos datos que avanzan con el tiempo para la misma solicitud (`D-106`). El borrado es de rol `operaciones`, no `admin`, porque no hay historial de gestión que perder --se puede borrar de a una o **vaciar la tabla entera** para volver a cargar desde cero, y esto último es recuperable: reimportar el mismo archivo repone exactamente lo que había--. El modal de carga trae la misma estructura desplegable y la misma plantilla descargable que Canjes --tampoco se llena a mano, sirve para comparar encabezados cuando la carga falla--, con **solo `Propiedad` marcada como obligatoria** (`D-105`).

**Cobranza, y sin un gran total.** Pantalla propia con todo lo facturable y lo pagable de los dos mundos, agrupado por parte y por estado. **No hay ningún total general y es deliberado:** las seis partes de un negocio son dos niveles de la misma plata --la comisión total se reparte, y lo que le queda a ViveProp se reparte otra vez-- así que sumarlas contaría lo mismo dos veces. La de canjes va aparte porque es de Dataprop. **Y dentro de cada parte la plata va en tres columnas** --Ganado, En pipeline, No concretado; y en canjes **Ya facturado, Comisión de activos, No se llegó a cobrar**, donde solo la del medio es comisión de Dataprop (`D-102`)-- que tampoco se suman entre sí, igual que en el listado de Negocios (`D-063`). Un selector cambia entre lo **calculado** por el motor y lo **registrado**. La tabla se puede comprobar sumando: hay una fila para el **rebate del concentrador** --que entra a la comisión real VP sin salir de ninguna otra parte-- y un aviso cuando el reparto de alguna liquidación no cuadra con su comisión total, como en VVP-2 (`D-092`, `D-095`, `D-045`).

**El reporte semanal muestra el flujo del mes, con los meses anteriores encima.** Dos pestañas --`Canjes │ Negocios`--, un cursor de **mes** con flechas, y «Comparar con los últimos **1 a 12 meses**». El eje son **las semanas del mes**, contadas **desde el día 1** (`S1 1-7`, `S2 8-14`, y así) y no de lunes a domingo: con lunes, la primera semana se reparte con el mes anterior y «S1» dejaría de significar el arranque del mes. La última semana es parcial --tres días en un mes de 31-- así que **siempre se ve más baja**, y eso se dice arriba y en el globo del gráfico; febrero tiene cuatro semanas y las líneas de los meses de cinco **se cortan** ahí en vez de dibujar un cero. Van cuatro bloques y cada título es la pregunta que responde: el **flujo** --entraron / avanzaron de etapa / se cayeron-- en gráficos de **barras agrupadas**, con **una barra por cada mes seleccionado** --hasta doce-- y el elegido en **azul navy**. Hasta tres meses el color identifica el mes; de cuatro en adelante codifica **recencia** en un solo tono, porque doce tonos distinguibles no existen: ahí el color dice la dirección --fuerte es reciente-- y quién es cada barra lo dice la **posición**, fija en cada semana, más el globo y la tabla. De siete meses los gráficos bajan a dos columnas y de diez a una, para que cada barra siga siendo una barra (`D-099`, `D-101`); cada gráfico lleva **una sola línea de tendencia**, ajustada con el promedio de cada semana en **toda la ventana comparada** --así el filtro de meses la cambia-- y trazada solo sobre las **semanas completas**, porque la parcial la haría bajar al final siempre (`D-100`); **«Los mismos números»**, la misma cosa en tabla con un mes por fila y su variación contra el elegido; **«Por dónde avanzaron»**, cuántos entraron a cada etapa contra el promedio de los meses comparados; **«Dónde está lo abierto hoy»** --que es la foto de hoy y lo dice, con la comisión parada en cada etapa y cuánto lleva--; y **«Mes a mes»**, con los totales del período y la curva de tendencia. **La plata de ese bloque va solo en negocios** (`D-103`): en canjes, un monto en la fila del mes se lee como lo hecho ese mes, cuando dice cuánto habrían valido canjes que en su mayoría se cancelaron --la comisión de canjes en juego es la de los activos, y esa se muestra en «Dónde está lo abierto hoy»--. La plata va por etapa y por mes y **no** por semana, porque la comisión se gana al cerrar. En negocios, «Avanzaron» y «Se perdieron» **explican por qué están vacíos** --el pipeline no tiene movimientos registrados y las liquidaciones perdidas no tienen fecha de cierre-- y aparecen solos en cuanto haya datos: una línea en cero diría «no pasó nada» y lo que pasa es «no se sabe» (`D-098`).
